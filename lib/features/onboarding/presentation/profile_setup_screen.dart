...
class _ProfileSetupScreenState extends State<ProfileSetupScreen>
  ...
  Future<void> _saveAndFinish() async {
    ...
    final team = _selectedTeamId != null ? kTeams.firstWhere((t) => t.id == _selectedTeamId) : null;
    final driver = _selectedDriverId != null ? kDrivers.firstWhere((d) => d.id == _selectedDriverId) : null;
  ...
  String get _stepLabel => const ['YOUR NAME', 'PICK YOUR TEAM', 'PICK YOUR DRIVER'][_step];
              ...
              children: [
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: FadeTransition(opacity: _fadeAnim, child: _buildStepContent()),
                  ),
                ),
                _buildFooter(),
              ],
      ...
      child: Column(
        children: [
          AnimatedBuilder(
            ...
            builder: (_, __) => Text('◆  STEP ${_step + 1} OF 3  ◆', style: GoogleFonts.orbitron(
              fontSize: 10, letterSpacing: 4, color: _accentColor.withOpacity(_pulseAnim.value),
            )),
          ),
          const SizedBox(height: 10),
          Text(_stepLabel, style: GoogleFonts.orbitron(
            fontSize: 26, fontWeight: FontWeight.w900, color: _white, letterSpacing: 2,
          )),
            ...
            children: List.generate(3, (i) {
              ...
              return AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  color: done || active ? _accentColor : _white.withOpacity(0.15),
                  boxShadow: active ? [BoxShadow(color: _accentColor.withOpacity(0.6), blurRadius: 6)] : null,
                ),
              );
            }),
          ),
          ...
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [
                Colors.transparent, _accentColor.withOpacity(0.6), Colors.transparent,
              ]),
            ),
          ),
      ...
      child: Column(
        children: [
          AnimatedContainer(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: [
                Colors.transparent, _accentColor.withOpacity(0.3), Colors.transparent,
              ]),
            ),
          ),
          GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: _accentColor.withOpacity(0.3), blurRadius: 16)],
              ),
              child: Center(
                child: Text(_ctaLabel, style: GoogleFonts.orbitron(
          ...
          if (_step > 0) ...[
            ...
            GestureDetector(
              ...
              child: Text('‹  BACK', style: GoogleFonts.orbitron(
                fontSize: 10, letterSpacing: 3, color: _white.withOpacity(0.25),
              )),
            ),
          ],
        ],
  ...
  Widget _buildNameStep() {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const SizedBox(height: 8),
        Text('What should we call you on the grid?', style: GoogleFonts.rajdhani(
          fontSize: 15, color: _white.withOpacity(0.5), letterSpacing: 0.5,
        )),
        const SizedBox(height: 24),
        Container(
          decoration: BoxDecoration(
            border: Border.all(
              color: _nameError != null ? const Color(0xFFFF073A) :
              _nameController.text.isNotEmpty ? _accentColor.withOpacity(0.8) :
              _white.withOpacity(0.15),
            ),
            borderRadius: BorderRadius.circular(4),
            color: _white.withOpacity(0.03),
            boxShadow: _nameController.text.isNotEmpty ? [BoxShadow(
                color: _accentColor.withOpacity(0.1), blurRadius: 12)] : null,
          ),
          child: TextField(
            ...
            style: GoogleFonts.orbitron(fontSize: 16, color: _white, letterSpacing: 2),
            decoration: InputDecoration(
              hintText: 'e.g. VERSTAPPEN FAN',
              hintStyle: GoogleFonts.orbitron(fontSize: 13, color: _white.withOpacity(0.2), letterSpacing: 2),
        ...
        if (_nameError != null) ...[
          ...
          Row(children: [
            ...
            Text(_nameError!, style: GoogleFonts.rajdhani(fontSize: 13, color: const Color(0xFFFF073A))),
          ]),
        ],
        ...
        Container(
          ...
          decoration: BoxDecoration(
            border: Border(left: BorderSide(color: _accentColor.withOpacity(0.5), width: 2)),
          ),
          child: Text(
            'Your name will appear on the leaderboard during race simulations.',
            style: GoogleFonts.rajdhani(fontSize: 13, color: _white.withOpacity(0.35), height: 1.6),
          ),
        ),
      ]),
    );
  ...
  Widget _buildTeamStep() {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 24),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Choose your constructor allegiance', style: GoogleFonts.rajdhani(
          fontSize: 15, color: _white.withOpacity(0.5),
        )),
        const SizedBox(height: 16),
        GridView.builder(
          shrinkWrap: true,
  ...
  Widget _buildTeamCard(F1Team team) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          border: Border.all(color: selected ? team.color : _white.withOpacity(0.1), width: selected ? 1.5 : 1),
          borderRadius: BorderRadius.circular(4),
          color: selected ? team.color.withOpacity(0.1) : _white.withOpacity(0.02),
          boxShadow: selected ? [BoxShadow(color: team.color.withOpacity(0.25), blurRadius: 12)] : null,
        ),
        child: Row(children: [
          ...
          Expanded(child: Text(team.name, style: GoogleFonts.rajdhani(
            ...
            color: selected ? _white : _white.withOpacity(0.6), letterSpacing: 0.3,
          ), maxLines: 1, overflow: TextOverflow.ellipsis)),
          if (selected) Text('✓', style: TextStyle(fontSize: 12, color: team.color)),
        ]),
      ),
    );
  }
  ...
  Widget _buildDriverStep() {
    ...
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 24),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(team != null ? '${team.name} drivers shown first' : 'Pick your favourite driver',
          style: GoogleFonts.rajdhani(fontSize: 15, color: _white.withOpacity(0.5)),
        ),
        const SizedBox(height: 12),
        ListView(
          shrinkWrap: true,
                ...
                child: Row(children: [
                  ...
                  Padding(
                    ...
                    child: Text('OTHER DRIVERS', style: GoogleFonts.orbitron(
                      fontSize: 9, letterSpacing: 2, color: _white.withOpacity(0.2),
                    )),
                  ),
                  Expanded(child: Divider(color: _white.withOpacity(0.08))),
                ]),
  ...
  Widget _buildDriverTile(F1Driver driver, {required bool highlight}) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          border: Border.all(color: selected ? team.color : _white.withOpacity(highlight ? 0.1 : 0.05), width: selected ? 1.5 : 1),
          borderRadius: BorderRadius.circular(4),
          color: selected ? team.color.withOpacity(0.1) : _white.withOpacity(highlight ? 0.03 : 0.01),
          boxShadow: selected ? [BoxShadow(color: team.color.withOpacity(0.2), blurRadius: 10)] : null,
        ),
        child: Row(children: [
          ...
          Expanded(child: Column(
            ...
            children: [
              ...
              Text(team.name, style: GoogleFonts.rajdhani(
                fontSize: 12, color: selected ? team.color : _white.withOpacity(0.3),
              )),
            ],
          )),
          Text('#${driver.number}', style: GoogleFonts.orbitron(
            fontSize: 14, fontWeight: FontWeight.w900, color: team.color.withOpacity(selected ? 0.9 : 0.4),
          )),
          if (selected) ...[
            const SizedBox(width: 8),
...
class _ParticleBackgroundState extends State<_ParticleBackground> with SingleTickerProviderStateMixin {
  ...
  static final _particles = List.generate(60, (i) => _Particle(
    x: _rng.nextDouble(), y: _rng.nextDouble(), speed: 0.04 + _rng.nextDouble() * 0.1,
    radius: 0.4 + _rng.nextDouble() * 1.2,
    color: [const Color(0xFF00E5FF), const Color(0xFFFF00FF), const Color(0xFF39FF14), const Color(0xFFFFE600)][i % 4].withOpacity(0.35 + _rng.nextDouble() * 0.25),
  ));
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
    animation: _ctrl,
    builder: (_, __) => CustomPaint(painter: _ParticlePainter(_ctrl.value, _particles), size: MediaQuery.of(context).size),
  );
}

class _Particle {
  final double x, y, speed, radius; final Color color;
  const _Particle({required this.x, required this.y, required this.speed, required this.radius, required this.color});
}

class _ParticlePainter extends CustomPainter {
  final double progress; final List<_Particle> particles;
  _ParticlePainter(this.progress, this.particles);
  @override
  void paint(Canvas canvas, Size size) {
    for (final p in particles) {
      final y = ((p.y - progress * p.speed) % 1.0) * size.height;
      canvas.drawCircle(Offset(p.x * size.width, y), p.radius, Paint()..color = p.color);
    }
  }
  @override
...
class _CornerBracket extends StatelessWidget {
  final double size; final Color color; final Corner corner;
  const _CornerBracket({required this.size, required this.color, required this.corner});
  @override
  Widget build(BuildContext context) => CustomPaint(size: Size(size, size), painter: _CornerPainter(color: color, corner: corner));
}

class _CornerPainter extends CustomPainter {
  final Color color; final Corner corner;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path();
    switch (corner) {
      case Corner.topLeft:     path.moveTo(w, 0); path.lineTo(0, 0); path.lineTo(0, h); break;
      case Corner.topRight:    path.moveTo(0, 0); path.lineTo(w, 0); path.lineTo(w, h); break;
      case Corner.bottomLeft:  path.moveTo(0, 0); path.lineTo(0, h); path.lineTo(w, h); break;
      case Corner.bottomRight: path.moveTo(w, 0); path.lineTo(w, h); path.lineTo(0, h); break;
    }
